...
```
[Jira 카드: claude-work 포함 + 담당자=나 + 상태≠완료]
...
`TRIGGER_TEXT`(기본 claude-work), `DONE_STATUS`(기본 DEV COMPLETED),
`PLANNED_LABEL`(기본 claude-planned), `ENV_SRC`(기본 work.env), `CLONE_BASE`(기본 repos/),
`LOOP_INTERVAL`(기본 3600), `ANTHROPIC_API_KEY`, `GH_TOKEN`
