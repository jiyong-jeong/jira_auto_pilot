...
for OR in "${R_OWNER[@]}"; do
  # 이 이슈의 '열린' PR 목록(draft 제외)
  PRS_JSON="$(gh pr list --repo "${OR}" --search "${ISSUE_KEY}" --state open --json number,url,isDraft,headRefName 2>/dev/null || echo '[]')"
  NUMS="$(printf '%s' "${PRS_JSON}" | node -e 'let d="";process.stdin.on("data",c=>d+=c).on("end",()=>{try{JSON.parse(d).filter(p=>!p.isDraft).forEach(p=>console.log(p.number))}catch{}})')"
  [[ -z "${NUMS}" ]] && continue
