...
if [[ "${MODE}" == "plan" ]]; then
  ...
  # review: PR 을 올린(claude-pr) 카드 = 병합 대기 PR → 자동 리뷰 대상
  JQL="assignee = currentUser()${DONE_FILTER} AND ${TRIGGER_FILTER} AND labels = \"${PR_OPEN_LABEL}\"${FAILED_FILTER}${PROJECT_FILTER}"
else
  echo "Usage: $0 <plan|build|review>" >&2
  exit 1
fi
