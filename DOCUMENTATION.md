...
6. clone 디렉토리로 `cd` 후 `claude -p "<프롬프트>"` 실행.
   ...
   - **build 프롬프트**: 진입 게이트(둘 다 필요) — (a) `claude-answered` 라벨, (b) 담당자 실제 답변 코멘트 — 하나라도 없으면 `SKIP: awaiting answers`. 통과 시 구현 → 브랜치/커밋/푸시 → PR → 완료 요약 기입 → 상태 전환. env 파일은 절대 커밋 금지 지시 포함.
...
- **build 대상 JQL**: `assignee = currentUser() AND status != "DEV COMPLETED" AND text ~ "claude-work" AND labels = "claude-planned" AND labels = "claude-answered" AND (labels != "claude-failed" OR labels IS EMPTY)`
