...
빌드 도구 없이 CDN(React + Tailwind)으로 동작하는 단일 페이지. 루프 제어를 제외한 모든 섹션은 제목 클릭으로 접기/펼치기됩니다(설정·자격증명·work.env·카드 등록은 기본 접힘, 카드 상태·처리 이력·실시간 로그는 기본 펼침). 섹션 구성:

- **루프 제어**: plan/build 시작·중지, **전체 실행**(준비된 모든 카드 즉시 1회), 실행 상태(pid). "전체 실행"은 스케줄과 무관하게 detect→처리 한 사이클을 돌려 같은 로그에 출력합니다.
- **프로젝트 설정**(접이식, 기본 접힘): repo·Jira·담당자·트리거·완료상태·라벨·주기·clone베이스·test/build 명령 등 → 저장 시 `config.json`. 제목 클릭으로 펼침/접힘.
  - **Jira 카드 등록 · 카드 상태**: 프로젝트에 종속되므로 프로젝트 설정 섹션 안의 하위 블록으로 통합됨(설정을 접으면 함께 접힘). 카드 등록(이슈타입·상위·이미지·Claude 정리)·카드 상태(단계 표·펼침 상세·답변·단건 실행·Claude 로그) 기능은 동일.
  - **환경 변수(.env)**: 프로젝트 설정에 통합됨. `envMode` 로 두 방식 중 선택 — **파일 경로 지정**(`path`): 지정한 파일(`envPath`)을 그대로 대상 repo 의 `.env`(work.env)로 복사 / **내용 직접 입력**(`content`, 기본): 텍스트로 입력한 내용을 `envPath`(비우면 `<WORK_DIR>/work.env`)에 저장해 사용. 두 경우 모두 `ENV_SRC=envPath` 로 clone 에 복사됩니다.
- **자격증명**(접이식, 기본 접힘): Anthropic / GitHub / Atlassian 토큰 + Slack Incoming Webhook 저장(마스킹, 빈 입력은 기존값 유지). 제목 클릭으로 펼침/접힘. 각 입력란의 `ⓘ` 아이콘에 마우스를 올리면 해당 토큰의 용도 설명 말풍선이 표시됩니다.
