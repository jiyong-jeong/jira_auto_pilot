...
[plan 완료]  claude-planned 라벨 있음
      ...
      │         - base 브랜치 대상 PR 생성
      │         - 완료 요약 기입(label 모드: 코멘트 + 설명 맨 아래 '## 완료 내역' / text 모드: 트리거 텍스트 바로 위)
...
6. clone 디렉토리로 `cd` 후 `claude -p "<프롬프트>"` 실행.
   ...
   - **build 프롬프트**: 진입 게이트(둘 다 필요) — (a) `claude-answered` 라벨, (b) 담당자 실제 답변 코멘트 — 하나라도 없으면 `SKIP: awaiting answers`. 통과 시 구현 → **PR 전 검증** → 브랜치/커밋/푸시 → PR → 완료 요약 기입 → 상태 전환. env 파일은 절대 커밋 금지 지시 포함. (완료 요약은 label 모드에서 **코멘트 + 카드 설명 맨 아래 '## 완료 내역'** 두 곳에, text 모드에서 트리거 텍스트 바로 위에 작성. 변경 요약·PR·브랜치·완료 일시 포함)
...
- **프로젝트 카드**(접이식, `ProjectCard`): 제목=이름·프로젝트키. 내부에:
  - **설정**: repo·Jira·프로젝트키·담당자·트리거·완료상태·라벨·주기·동시상한·clone베이스·test/build 명령 → "설정 저장"(`POST /api/projects`).
  ...
  - **Jira 카드 등록 / 카드 상태**: 아래 설명과 동일, 모두 `?project=<id>` 스코프.
