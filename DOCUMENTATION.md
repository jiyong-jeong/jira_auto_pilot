...
매 실행 종료 시 결과(성공/스킵/실패 + PR URL·브랜치)를 `HISTORY_FILE`(기본 `history.jsonl`)에 JSONL 한 줄로 기록한다. **브랜치명은 PR URL 로 `gh pr view --json headRefName` 을 조회해 PR 의 실제 head 브랜치를 사용**하므로 `feat/`·`fix/` 등 접두사에 무관하게 정확히 기록된다(조회 실패 시 claude 출력에서 접두사 포괄 추출로 폴백). 대시보드 병합(`/api/cards/:key/merge`)도 동일하게 병합된 PR 의 head 브랜치를 이력에 남긴다.
...
- **PR 해석**: 카드 KEY 로 각 대상 repo 의 열린 PR(draft 제외)을 `gh pr list --search <KEY>` 로 찾는다.
...
| POST | `/api/cards/:key/merge` | 카드의 PR(들)을 `gh pr merge --rebase --delete-branch` 로 병합(이미 GitHub 에서 병합된 PR 도 인정). 1건 이상 병합되면 카드를 완료 상태로 전환 + `claude-pr` 라벨 제거 + 처리 이력에 `merge/merged` 기록 + **카드의 clone 디렉토리(`<CLONE_BASE>/<repo>-<KEY>`) 삭제**(디스크 회수, 처리 중 lock 이면 생략). 응답에 `removed[]`(삭제된 디렉토리명) 포함 |
| POST | `/api/cards/sync-merged` | **외부(대시보드 밖) 병합 자동 반영**: await-merge(`claude-pr`) 카드 중 PR 이 모두 MERGED(열린 PR 0)면 재병합 없이 완료 처리(상태 전환·라벨 제거·clone 정리·이력 기록). `project` 지정 시 해당 프로젝트, 없으면 전 프로젝트. 응답 `completed[]`. 백엔드가 **3분 주기로 자동 실행**하며, 대시보드 "병합 동기화" 버튼으로 즉시 트리거 |
