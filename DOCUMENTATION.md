...
- **plan 대상 JQL**: `assignee = currentUser() AND status != "DEV COMPLETED" AND text ~ "claude-work" AND (labels != "claude-planned" OR labels IS EMPTY)`
- **build 대상 JQL**: `assignee = currentUser() AND status != "DEV COMPLETED" AND text ~ "claude-work" AND labels = "claude-planned"`
...
- **실패 처리**: 실패 카드가 무한 재시도 → `claude-failed` 라벨 + 백오프 + 실패 코멘트.
