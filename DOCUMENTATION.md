...
| POST | `/api/loops/:type/run-once` | 즉시 1회 실행(스케줄 무시, 같은 로그에 기록) |
...
- **루프 제어**: plan/build 시작·중지, **지금 실행**(즉시 1회), 실행 상태(pid). "지금 실행"은 스케줄과 무관하게 즉시 한 사이클을 돌려 같은 로그에 출력합니다.
- **프로젝트 설정**: repo·Jira·담당자·트리거·완료상태·라벨·주기·env경로·clone베이스 등 → 저장 시 `config.json`.
- **자격증명**: Anthropic / GitHub / Atlassian 토큰 + Slack Incoming Webhook 저장(마스킹, 빈 입력은 기존값 유지). 각 입력란의 `ⓘ` 아이콘에 마우스를 올리면 해당 토큰의 용도 설명 말풍선이 표시됩니다.
- **work.env 파일**: env 내용을 불러와 편집·저장(저장 시 `.bak` 백업).
- **Jira 카드 등록**: 이슈 타입(프로젝트 메타에서 조회)·요약·설명을 입력해 카드를 생성. 상위는 **에픽 드롭다운**(프로젝트 에픽 목록) 또는 **상위 키 직접 입력**(우선 적용)으로 지정(`fields.parent`). 체크박스로 트리거 라벨(`claude-work`) 추가와 본인 할당을 선택 — 둘 다 켜면 생성 즉시 자동화(plan) 대상이 됩니다. 설명은 ADF 로 변환되어 전송됩니다.
