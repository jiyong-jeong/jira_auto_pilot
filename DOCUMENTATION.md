...
- **프로젝트 카드**(접이식, `ProjectCard`): 제목=이름·프로젝트키. 내부에:
  ...
  - **Jira 카드 등록 / 카드 상태**: 아래 설명과 동일, 모두 `?project=<id>` 스코프. 카드 단계는 라벨/상태 기반(질문대기/답변대기/개발대기/실패/완료)에 더해, **처리 락이 있으면 "처리 중(plan|build)"** 으로 표시(실행 중 반영). 카드 상태는 **마운트 시 1회 + 60초마다 자동 갱신**(프로젝트 키 있을 때), 수동 "조회" 버튼도 그대로 사용.
