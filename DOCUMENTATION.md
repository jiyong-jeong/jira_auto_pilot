...
| 처리 락(`repos/.state/<KEY>.lock` + `.lock.phase`/`.lock.pid`) | 해당 카드 plan/build **처리 중**(phase·스크립트 PID 기록 → 대시보드 '중지'에 사용) | run-jira-claude.sh(시작 시 생성, 종료 시 trap 으로 제거) |
...
- 코드/커밋/머지/상태·라벨은 **건드리지 않음**(리뷰 코멘트만). 결과는 이력에 `review/approved` 또는 `review/reviewed` 로 기록.
...
| POST | `/api/cards/:key/stop` | 처리 중인 카드의 claude 작업 중지(락 PID 의 프로세스 트리 `run-jira-claude.sh→claude→도구` 를 SIGTERM→4초후 SIGKILL). 루프/다른 카드는 무영향. 이력에 `stopped` 기록 |
...
- **처리 이력**(전역): `history.jsonl` 기반 최근 처리 내역(시각·**프로젝트**·키·단계·결과·PR·브랜치) 표. 4초마다 갱신. **페이지당 10건 페이지네이션**(처음/이전/다음/마지막 + "n / 총페이지", 총 건수·표시 범위 표기). 갱신으로 건수가 변해도 현재 페이지를 안전하게 클램프.
