...
- **카드 상태**: 트리거 카드의 단계(plan대기/답변대기/build대기/실패/완료) 표. **행을 클릭하면 아래로 펼쳐져** 원문 설명과 코멘트(특히 plan 단계의 질문)를 보고, 그 자리에서 **답변을 코멘트로 등록**할 수 있습니다. "답변 완료로 표시" 체크 시 `claude-answered` 라벨도 함께 추가되어 다음 주기에 build 가 진행됩니다.
  ...
  - **리뷰 반영(PR 갱신)**: PR 리뷰 후 재개발이 필요할 때 사용. (선택)메모를 적고 "리뷰 반영 실행"을 누르면, 메모가 있으면 Jira 코멘트로 남긴 뒤 `REWORK` 모드로 build 를 돌린다. claude 는 **새 PR 을 만들지 않고** 이슈의 기존 PR 브랜치를 checkout → GitHub PR 리뷰 코멘트 + Jira 코멘트를 반영 → 같은 브랜치에 push(기존 PR 갱신). 상태는 바꾸지 않음. 스케줄 루프는 rework 를 하지 않음(명시적 단건 전용). 결과는 이력에 `rework` 로 기록.
